...
class Pathology(ExtendedVisionDataset):
    ...
    def _get_entries(self) -> np.ndarray:
        ...
        # Read entry.csv once if needed by any of the following blocks
        _entry_dataset_df = None
        if self._entry_dataset is None or self._entry_slide is None:
    ...
    def _dump_entries(self) -> None:
        ...
        for _, row in entry_df.iterrows():
            ...
            for patches_dir in patches_dirs:
                ...
                try:
                    # aa = [f for f in os.listdir(patches_path) if f.lower().endswith((".jpeg"))]
                    aa = os.listdir(patches_path)
                    for f in aa:
                        file_path = os.path.join(patches_path, f)#对于每一个坐标文件夹
                        # try:
                        #     # 检查文件是否存在且可访问
                        #     os.stat(file_path)
                        #     # 尝试打开图片验证是否为有效图像文件
                        ...
                        #         a=1
                        #         # img.verify()  # 只检查文件是否能被完整解码，不加载内容
                        # except (OSError, UnidentifiedImageError) as fe:
                        #     raise Exception(f"Corrupted image file detected: {file_path} ({fe})")
                        # except Exception as fe:
                        #     raise Exception(f"Corrupted file detected: {file_path} ({fe})")
                except Exception as e:
                    print(f"Skipping {patches_path} due to: {e}")
                    continue
                sample_count += len(aa)
        ...
        for idx, row in entry_df.iterrows():
            ...
            for dir_index, patches_dir in enumerate(patches_dirs):
                patches_path = os.path.join(patches_dirs_path, patches_dir)
                try:
                    # bb = [f for f in os.listdir(patches_path) if f.lower().endswith((".jpeg"))]
                    bb = os.listdir(patches_path)
                    for f in bb:
                        file_path = os.path.join(patches_path, f)
                        # try:
                        #     # 检查文件是否存在且可访问
                        #     os.stat(file_path)
                        #     # 尝试打开图片验证是否为有效图像文件
                    ...
                    print(f"Skipping {patches_path} due to: {e}")
                    continue
                for patch_file in bb:#文件夹跳过了，文件再跳过！
        ...
        import math
        print("总数:",sample_count)
        epoch_length = sample_count // (128 * 4 * 100) 
        epoch_length = math.ceil(epoch_length / 50) * 50 #50的倍数上取整
        print(f'saving entries to "{self._entries_path}"')
        print("为了让模型至少见过每张图片一次，需满足：")
        print("batch_size * epochs * epoch_length > dataset_size")
        print("其中：batch_size = batch_size_per_gpu * gpu_num = 128 * 4 = 384; epochs = 100")
